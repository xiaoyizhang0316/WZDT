...
public class FTEController : MonoBehaviour , ICanvasRaycastFilter
{
    ...
    public void Guide(Canvas canvas, RectTransform target, GuideType guideType,float scale,float time, TranslateType translateType = TranslateType.Direct, float moveTime = 1) {
        ...
        {
            case GuideType.Rect:
                rectGuide.Move(canvas, target,scale,time,translateType,moveTime);
                break;
            case GuideType.Circle:
                circleGuide.Move(canvas, target,scale,time, translateType, moveTime);
                break;
        }
    }
