...
public class HealthBarSet : MonoBehaviour
{
    ...
    public void SetBar(List<int> stageDan)
    {
        ...
        bar2.sizeDelta = new Vector2(3+(stageDan[1]-stageDan[0]) / (float)stageDan[2] * CommonParams.bloodTotalWidth, height2);
        bar3.sizeDelta = new Vector2(3+(stageDan[2]-stageDan[1]) / (float)stageDan[2] * CommonParams.bloodTotalWidth, height3);
    }
}
