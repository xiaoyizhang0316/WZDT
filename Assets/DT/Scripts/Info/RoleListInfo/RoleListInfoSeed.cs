...
public class RoleListInfoSeed : BaseRoleListInfo
{
    ...
    public void ShowLastpruduct(Role role)
    {
        ...
        for (int i = 0; i <baseMapRole.GetComponent<ProductSeed>().productDatas.Count; i++)
        {
            data = baseMapRole.GetComponent<ProductSeed>().productDatas[i];
        }
        ...
        Pruductgame.GetComponent<InitDanzhongPrb>().Init(RoleUpdateInfo.My.seedSpeed,data.damage.ToString(),data.loadingSpeed.ToString(),data);
    }
    //public void ShowLastpruduct(Role role)
   //{
   //    BaseMapRole baseMapRole = PlayerData.My.GetMapRoleById(role.ID);
