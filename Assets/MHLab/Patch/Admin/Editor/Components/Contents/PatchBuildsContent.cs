...
namespace MHLab.Patch.Admin.Editor.Components.Contents
{
    public class PatchBuildsContent : PatchContent
    {
        ...
        private void UpdateReleaseTypes()
        {
            ...
            var version = new Version(currentVersion.Major, currentVersion.Minor, currentVersion.Build + 1);
            _releaseTypesText[0] = $"{_releaseTypes[0]}: {version}";
            
            version = new Version(currentVersion.Major, currentVersion.Minor + 1, 0);
            _releaseTypesText[1] = $"{_releaseTypes[1]}: {version}";
            
            version = new Version(currentVersion.Major + 1, 0, 0);
            ...
            if (currentVersion != null)
            {
                switch (_releaseTypeIndex)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        currentVersion = new Version(currentVersion.Major, currentVersion.Minor + 1, 0);
                        break;
                    case 2:
                        currentVersion = new Version(currentVersion.Major + 1, 0, 0);
                        break;
                }
            }
            else
